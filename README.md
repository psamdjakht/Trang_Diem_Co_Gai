...
## Mobile GitHub Pages edition in this package
This package includes a root `index.html` ready for GitHub Pages, native Pointer Events for stable phone dragging, automatic snap positions for the included clothes, scroll locking while dragging, tap-to-wear, and an extensible configuration in `scripts/dollmaker-config.js`. Vietnamese deployment and extension instructions are in `HUONG_DAN_GITHUB_VA_MO_RONG.md`. Historical sections below may still mention jQuery UI or Touch Punch; the root static page now uses native Pointer Events instead.
